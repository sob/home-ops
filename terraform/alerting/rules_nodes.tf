...
resource "grafana_rule_group" "node_availability" {
  ...
  # Alert when multiple nodes are down (using blackbox ICMP probes)
  rule {
    name        = "MultipleNodesDown"
    annotations = {
      summary     = "Multiple Kubernetes nodes are down"
      description = "$${values.A} nodes are currently unreachable - cluster stability at risk!"
    }
    labels = {
      severity = "critical"
    }
    ...
    data {
      ...
      datasource_uid = local.prometheus_cloud_uid
      model          = jsonencode({
        expr = "count(probe_success{probe_type=\"node_icmp\", instance=~\"10.1.100.10[1-7]\"} == 0) >= 2"
        refId = "A"
        instant = true
      })
    }
  ...
  rule {
    ...
    annotations = {
      ...
      description = "Control plane node $${labels.instance} has been unreachable for 3 minutes"
    }
    labels = {
      severity = "critical"
    }
